...
Route::middleware(config('nova-file-manager.middleware', ['nova', Authenticate::class]))
    ->prefix(config('nova-file-manager.route_prefix', 'nova-vendor/unifilemanager/nova-file-manager'))
    ->name('nova-file-manager.')
    ->group(function (): void {
        Route::get('/storage-areas', [FileManagerController::class, 'storageAreas'])->name('storage-areas.index');
        ...
        Route::get('/items/move-destinations', [FileManagerController::class, 'moveDestinations'])->name('items.move-destinations');
        ...
        Route::get('/preview', [FileManagerController::class, 'preview'])
            ->middleware('throttle:nova-file-manager-previews')
            ->name('items.preview');
        Route::get('/download', [FileManagerController::class, 'download'])->name('items.download');
        Route::get('/selectable-file', [FileManagerController::class, 'selectableFile'])->name('items.selectable-file');
    });
